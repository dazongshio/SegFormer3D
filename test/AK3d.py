...
class AKConv(nn.Module):
    def __init__(
        ...
        self.conv = nn.Sequential(nn.Conv3d(in_channel, embed_dim, kernel_size=kernel_size, padding=padding,stride=stride, bias=False),
                                  nn.BatchNorm3d(embed_dim),
                                  nn.SiLU())  # the conv adds the BN and SiLU to compare original Conv in YOLOv5.
        self.p_conv = nn.Conv3d(in_channel, 3 * num_param, kernel_size=3, padding=1, stride=1)
...
if __name__ == '__main__':
    a = torch.ones(4, 4, 72, 72, 72)  #生成随机数
    b = AKConv(4,16,3,1,1,64)  #实例化
    c = b(a)
    print(c.size())
