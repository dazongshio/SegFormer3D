...
class MSDeformAttnPixelDecoder3D(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        srcs_res5 = self.decoder_res5(res5_features)#(2, 120, 12, 12,12)
        srcs_res4 = self.decoder_res4(res4_features)#(2,12,24,24,24)
        srcs_res3 = self.decoder_res3(res3_features)#(2,240,48,48,48)
...
if __name__ == "__main__":
    ...
    # Example usage
    input_tensor = {
        "res3": torch.randn(2, 96, 48, 48,48).to(device),
        "res4": torch.randn(2, 192, 24, 24,24).to(device),
        "res5": torch.randn(2, 384, 12, 12,12).to(device),
    ...
    decoder = MSDeformAttnPixelDecoder3D(input_shape=backbone_feature_shape).to(device)
